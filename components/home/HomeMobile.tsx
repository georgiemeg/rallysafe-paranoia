...
export function HomeMobile() {
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      if (!v) {
        setVv({ h: window.innerHeight, top: 0 });
        return;
      }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        <section className="mb-6 rounded-2xl bg-brand-teal p-5 sm:p-6">
          ...
          {loadingEvents ? (
            ...
            <select
              className="w-full bg-brand-ink border border-white/10 rounded-lg px-4 py-3 text-neutral-100 font-mono text-base focus:outline-none focus:ring-2 focus:ring-brand-gold"
          ...
          <>
            ...
            <section className="mb-6 rounded-2xl bg-brand-golden-orange p-5 sm:p-6">
              ...
              <input
                ...
                className="w-full max-w-sm bg-brand-ink border border-white/10 rounded-lg px-4 py-3 text-neutral-100 font-mono text-base focus:outline-none focus:ring-2 focus:ring-brand-gold"
                value={phone}
                ...
                <div className="rounded-xl border border-white/10 bg-brand-ink overflow-hidden">
                  <div className="px-4 py-3 border-b border-white/10 bg-white/[0.04]">
                    <input
                      ...
                      className="w-full bg-black/30 border border-white/10 rounded-md px-3 py-2 text-base text-neutral-100 focus:outline-none focus:ring-2 focus:ring-brand-gold"
                      value={search}
            ...
            <div className="sticky bottom-0 z-30 pt-4 pb-3 -mx-4 sm:-mx-6 px-4 sm:px-6 bg-gradient-to-t from-[#0a0e14] via-[#0a0e14]/95 to-transparent">
              {tracked.size > 0 && (
            ...
            {showInbox && (
              ...
              >
                ...
                <form
                  className="border-t border-white/10 p-3 flex gap-2 shrink-0 bg-[#0a0e14]"
                ...
                >
                  <input
                    ...
                    className="flex-1 min-w-0 bg-brand-ink border border-white/10 rounded-lg px-3 py-2 text-base text-neutral-100 focus:outline-none focus:ring-2 focus:ring-brand-gold"
                  />
                  <button
                    type="submit"
                    disabled={commandBusy || !commandText.trim()}
                    className="shrink-0 bg-brand-gold text-brand-ink font-bold rounded-lg px-4 py-2 text-base disabled:opacity-50"
                  >
                    {commandBusy ? "…" : "Send"}
                  </button>
                </form>
